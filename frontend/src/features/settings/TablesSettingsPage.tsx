...
import { DialogFooter, FieldError, ResourceDialog, Req } from "@/components/resource/dialog-kit";
...
export function TablesSettingsPage() {
  const restaurantId = useAppStore((state) => state.activeRestaurant!.id); const base = `/admin/restaurants/${restaurantId}/tables`;
  ...
  const deactivate = useMutation({ mutationFn: (id: string) => api(`${base}/${id}`, { method: "DELETE" }), onSuccess: () => void invalidate() });
  ...
  return <main className="page"><section className="resource-head"><span className="eyebrow"><Armchair size={14} /> Tables</span><Button onClick={() => open("new")}><Plus size={17} /> Add table</Button></section>
    {!query.data?.length ? <StateView title="No tables configured" message="Add physical tables to enable table-specific availability." /> : <div className="table-scroll"><table className="data-table"><thead><tr><th>Table</th><th>Capacity</th><th>Preference</th><th>Section</th><th>Status</th><th /></tr></thead><tbody>{query.data.map((table) => <tr key={table.id}><td>{table.name}</td><td>{table.capacity}</td><td>{labels[table.preference]}</td><td>{table.section || "—"}</td><td><span className="badge">{table.status.toLowerCase()}</span></td><td><button className="icon-button" onClick={() => open(table)}><Pencil size={14} /></button><button className="icon-button" onClick={() => deactivate.mutate(table.id)}><Trash2 size={14} /></button></td></tr>)}</tbody></table></div>}
